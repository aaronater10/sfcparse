...
def test1_xml_build_manual():
    ...
    xml_build = sfcparse.xmlbuildmanual()
    ...
    xml_to_str_data = sfcparse.xmlexportstr(xml_tree_root)
    xml_from_str_data = sfcparse.xmlimportstr(xml_to_str_data)

    sfcparse.xmlexportfile(filepath, xml_from_str_data)
    time.sleep(file_delay_timer)
    file_import = sfcparse.xmlimportfile(filepath)
