...
def test1_create_hash_file():
    ...
    sha1_returned = sfcparse.createfilehash(filepath_to_hash, filepath_to_cache, 'sha1')
    file_import = sfcparse.importfile(filepath_to_cache)
    ...
    md5_returned = sfcparse.createfilehash(filepath_to_hash, filepath_to_cache, 'md5')
    file_import = sfcparse.importfile(filepath_to_cache)
...
def test2_create_hash_only():
    ...
    # Test sha1 - generate hash of file and import to test value and type
    sha1_returned = sfcparse.createfilehash(filepath_to_hash, False, 'sha1')
    assert (sha1_returned == SHA1) and (isinstance(sha1_returned, str))

    # Test md5 - generate hash of file and import to test value and type
    md5_returned = sfcparse.createfilehash(filepath_to_hash, False, 'md5')
    assert (md5_returned == MD5) and (isinstance(md5_returned, str))